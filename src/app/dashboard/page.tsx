...
export default function Dashboard() {
  ...
  if (loading) return <main style={{minHeight:'100vh',background:'#0f0f11',display:'flex',alignItems:'center',justifyContent:'center',color:'#9090a8',fontFamily:'sans-serif'}}>Loading...</main>
    ...
    <main style={{minHeight:'100vh',background:'#0f0f11',fontFamily:'sans-serif'}}>
      <div style={{background:'#16161a',borderBottom:'1px solid rgba(255,255,255,0.07)',padding:'16px 28px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:'16px'}}>
          <span style={{color:'#9090a8',fontSize:'12px'}}>{user?.user_metadata?.agency_name || user?.email}</span>
          <button onClick={handleLogout} style={{background:'transparent',border:'1px solid rgba(255,255,255,0.12)',borderRadius:'6px',padding:'5px 12px',color:'#9090a8',fontSize:'12px',cursor:'pointer'}}>Log out</button>
        </div>
      </div>
      <div style={{padding:'40px 28px'}}>
        <h1 style={{color:'#e8e8f0',fontSize:'22px',fontWeight:'500',marginBottom:'8px'}}>Welcome to Creatorflow</h1>
        <p style={{color:'#9090a8',fontSize:'13px',marginBottom:'40px'}}>Your influencer campaign platform is ready.</p>
